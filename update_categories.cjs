...
// Fallback logic for categorizing based on name/tags/etc? We will just insert "category: 'xyz'" if missing.
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];

  if (line.match(/\s+{\s*$/)) { // matches "  {" or "{"
     // wait, the structure is const BUSINESS_CATEGORIES_INFO: BusinessCategoryInfo[] = [ { "id": "...", ... } ]
  }

  modifiedLines.push(line);
}

// Actually, AST approach or simple regex replace might be safer since it's JSON-like
let newContent = content.replace(/\{\s*"id":\s*"([^"]+)"(.*?)\}/gs, (match, id, rest) => {
  // If category is already there, maybe update it or skip
  if (match.includes('"category"')) {
    // replace it just in case
    return match.replace(/"category":\s*"[^"]+"/, `"category": "${CATEGORY_MAP[id] || 'specialized'}"`);
  } else {
    // Insert after "id": "..."
    const category = CATEGORY_MAP[id] || 'specialized';
    return match.replace(/"id":\s*"([^"]+)",/, `"id": "$1",\n    "category": "${category}",`);
  }
});

fs.writeFileSync('src/constants/businessRegistry.ts', newContent);

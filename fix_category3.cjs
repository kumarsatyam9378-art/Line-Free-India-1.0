...
const regex = /^ {4}"id": "([^"]+)",?\s*(?:\r?\n)(?:\s*"category": "[^"]+",?)?/gm;

let result = content.replace(regex, (match, id) => {
  const category = CATEGORY_MAP[id] || 'specialized';
  return `    "id": "${id}",\n    "category": "${category}",`;
});

// Since `category?: string;` might be removed or there are other occurrences we shouldn't touch, let's test if it worked well.
fs.writeFileSync('src/constants/businessRegistry.ts', result);

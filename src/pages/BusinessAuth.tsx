...
export default function BusinessAuth() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setRole('barber'); // Keep 'barber' for backward compatibility in backend
      // Check if business owner already has a profile → go home, else → setup
      const uid = (result as any)?.user?.uid || user?.uid;
      if (uid) {
        try {
          const snap = await getDoc(doc(db, 'barbers', uid)); // Keep 'barbers' collection for now
          if (snap.exists()) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 animate-fadeIn">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-primary to-accent flex items-center justify-center mb-6 shadow-xl">
          <span className="text-5xl">🏪</span>
        </div>
        <h1 className="text-2xl font-bold mb-2">Business Sign In</h1>
        <p className="text-text-dim mb-10 text-center">Manage your business & bookings online</p>

...
execSync('git checkout src/constants/businessRegistry.ts');
...
for (let i = 0; i < lines.length; i++) {
  ...
  const categoryMatchOriginal = line.match(/^ {4}"category": "([^"]+)",?$/);

  if (idMatch) {
    const id = idMatch[1];
    modifiedLines.push(line);

    // Default to 'specialized' if not explicitly defined above
    let category = CATEGORY_MAP[id];

    // Look ahead one line to see if there's an original category
    if (i + 1 < lines.length && lines[i + 1].match(/^ {4}"category": "([^"]+)",?$/)) {
      const origCat = lines[i + 1].match(/^ {4}"category": "([^"]+)",?$/)[1];
      // If we don't have a mapping for it, let's keep the original category, unless the original category was weird
      // If origCat is "home_services", we want it mapped to "home"
      // If origCat is "tech_&_it", we want it mapped to "technology"
      if (!category) {
  ...
  } else if (categoryMatchOriginal) {
    // Skip original category lines
    continue;
  } else {
    modifiedLines.push(line);
  }

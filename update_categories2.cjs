...
for (let i = 0; i < lines.length; i++) {
  ...
  // Match `  "id": "restaurant",` or `    "id": "restaurant",` but NOT in defaultServices (indentation level)
  // Actually, business categories have id with 4 spaces indent `    "id": "restaurant",`
  // defaultServices have id with 8 spaces indent `        "id": "1",`
  const match = line.match(/^ {4}"id": "([^"]+)",?$/);
  if (match) {
    const id = match[1];

    // Check if the next line already has category
    if (i + 1 < lines.length && lines[i+1].includes('"category":')) {
      // it already has it, we do nothing or replace
...
console.log('Done mapping categories with correct indentation!');

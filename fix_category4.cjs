...
for (let i = 0; i < lines.length; i++) {
  ...
  const categoryMatch = line.match(/^ {4}"category": "([^"]+)",?$/);

  if (idMatch) {
    currentId = idMatch[1];
    modifiedLines.push(line);
    // Add category line
    const category = CATEGORY_MAP[currentId] || 'specialized';
    modifiedLines.push(`    "category": "${category}",`);
  } else if (categoryMatch) {
    // Skip original category lines because we just added them under the id
    continue;
  } else {
    modifiedLines.push(line);
  }
